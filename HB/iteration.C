const double betaG = 0.006484206*1.00015*0.999995; //units of [kG/A]
const double leffG = 80.209; //units of [cm]
const double tol = 1E-5;
const double P0 = 0.00307690156; //units of [GeV/A]
...
double beta(double Iset){
  ...
  //from root:
  return (6.77648E-25*pow(Iset,6) - 8.43358E-21*pow(Iset,5) + 3.60792E-17*pow(Iset,4) - 6.38532E-14*pow(Iset,3) + 3.14903E-11*pow(Iset,2) - 3.06624E-08*Iset + 0.00650637);
...
double leff(double Iset){
  ...
  return leffG* (1 + (Iset>1246)*(2.018E-12*pow(Iset-1246,3)-9.44239E-9*pow(Iset-1246,2)-2.36088E-7*(Iset-1246)));
