double correctPosPol(double mkG){
  ...
  double corr = (-0.5969*mkG*mkG-9.6008*mkG-0.1447)/1000;
  return mkG - corr;
...
double correctNegPol(double mkG){
  ...
  double corr = (-0.5334*mkG*mkG+9.3121*mkG-0.9711)/1000;
  return mkG + corr;
...
void plotData(){
  ...
  //make output pdf
  TFile *fout = new TFile("output/hb.root","RECREATE");
  TCanvas *canvas = new TCanvas("canvas","hb data", 800, 800);
  ...
  std::string pdf_file_name = "output/hb.pdf";
  gROOT->SetBatch(true);
  ...
  TText *t_g1 = new TText(-3000,-5,"pos ramp 1");
  ...
  TText *t_g2 = new TText(-3000,-10,"pos ramp 2");
  ...
  TText *t_g3 = new TText(-3000,-15,"neg ramp");
  ...
  TText *t_asyn = new TText(500,-0.04,"not probe error corrected");
  TText *t_asyc = new TText(500,-0.05,"probe error corrected");
  ...
  gasyrel->Draw("ap");
  gasyrel->SetMinimum(-0.002);
  gasyrel->SetMaximum(0.0035);
  gasycrel->Draw("psame");
  t_asynr->Draw();
  t_asycr->Draw();
  canvas->Print( (pdf_file_name + "(").c_str());
  ...
  //plot B/I vs I
  const double beta_g = 0.00648593;
  ...
  gBI->SetMinimum(0.006);
  gBI->SetMaximum(0.0075);
  gBI->Draw("ap");
  TF1 *fBI = new TF1("fBI","pol6",20,4000);
  gBI->Fit(fBI,"R");
  ...
  betaRatioG->SetMinimum(0.95);
  betaRatioG->SetMaximum(1.01);
  betaRatioG->Draw("ap");
  TF1 *fBIrat = new TF1("fBIrat","pol6",100,4000);
  ...
  gl->SetTitle("Ratio of leff_{TOSCA,V10}/leff_{const}");
  gl->GetXaxis()->SetTitle("Current [A]");
  gl->GetYaxis()->SetTitle("leff_{TOSCA, V10} / leff_{const} [unitless]");
  gl->GetYaxis()->SetTitleOffset(2.4);
  gl->Draw("ap");
  
  TF1 *fleff = new TF1("fleff",fasdf,0,4000,5);
  fleff->FixParameter(0,1246);
  fleff->FixParameter(1,1);
  fleff->SetParameter(2,2.0475E-12);
  fleff->SetParameter(3,-1.724E-8);
  fleff->SetParameter(4,3.3395E-5);
