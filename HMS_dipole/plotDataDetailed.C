...
void plotDataDetailed(){

  //input the 3 axis HP data




  //input the central NMR data



  //input the external NMR data
  ...
  //~~~~~
  //fit B vs I for external NMR in range

  //fit B vs I for central NMR in range
  ...
  //make output pdf
  TFile *fout = new TFile("output/hms_dip.root","RECREATE");
  ...
  std::string pdf_file_name = "output/hms_dip.pdf";
  gROOT->SetBatch(true);
  ...
  //plot B vs I for all 3 probes and fit

  //plot the fit residual for each


  //plot the fit rel residual for each



  //plot the ratio of 3 axis to central and external to central
  ...
  gBI->Draw("ap");
  TF1 *fBI = new TF1("fBI","pol6",0,3000);
  gBI->Fit(fBI,"R");
