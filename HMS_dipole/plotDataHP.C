...
void plotDataHP(){
  ...
  r1->ReadFile("input/ramp1HP.txt","c1:ramp1");
  ...
  r2->ReadFile("input/ramp2HP.txt","c2:ramp2");
  ...
  r->ReadFile("input/rampHP.txt","c:ramp");
  ...
  //make output pdf
  TFile *fout = new TFile("output/hms_dipHP.root","RECREATE");
  ...
  std::string pdf_file_name = "output/hms_dipHP.pdf";
  gROOT->SetBatch(true);
  ...
  for (int ii=0; ii<nRamp1Pts;ii++){
	...
	cout<<"asy:\t"<<jj<<"\t"<<asyCorrRel[jj]<<""<<endl;
      }
    }
  }  
  ...
  gasy->Draw("ap");
  gasy->SetMinimum(-0.05);
  gasy->SetMaximum(0.05);
  gasyc->Draw("psame");
  // t_asyn->Draw();
  //t_asyc->Draw();
  canvas->Print( (pdf_file_name + "(").c_str());
  ...
  gasyrel->Draw("ap");
  gasyrel->SetMinimum(-0.004);
  gasyrel->SetMaximum(0.004);
  gasycrel->Draw("psame");
  //t_asynr->Draw();
  //t_asycr->Draw();
  canvas->Print( (pdf_file_name + "(").c_str());
