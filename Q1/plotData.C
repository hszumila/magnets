double correctNegPol(double mkG){
  ...
  double corr = (-0.5941*pow(mkG,2)-9.1599*mkG-0.2964)/1000;
  return mkG + corr;
...
double correctPosPol(double mkG){
  ...
  //double corr = (-0.4659*pow(mkG,2)+12.999*mkG-6.8839)/1000;
  double corr = (1.071E-06*pow(mkG,6) - 1.171E-04*pow(mkG,5) + 5.082E-03*pow(mkG,4) - 1.110E-01*pow(mkG,3) + 7.762E-01*pow(mkG,2) + 6.940E+00*mkG - 3.210E-01)/1000;
  return mkG - corr;
...
void plotData(){
  ...
  for (int jj=0;jj<nRamp1Pts; jj++){
    ...
    r1_B[jj] = r1BL->GetValue()/1000;//convert to kG
  ...
  for (int jj=0;jj<nRamp2Pts; jj++){
    ...
    r2_B[jj] = r2BL->GetValue()/1000;//convert to kG
  ...
  for (int jj=0;jj<nRampPts; jj++){
    ...
    r_B[jj] = rBL->GetValue()/1000;//convert to kG
    r_I[jj] = -rIL->GetValue();
  ...
  //make output pdf
  TFile *fout = new TFile("output/q1.root","RECREATE");
  TCanvas *canvas = new TCanvas("canvas","q1 data", 800, 800);
  ...
  std::string pdf_file_name = "output/q1.pdf";
  gROOT->SetBatch(true);
  ...
  g3->Draw("ap");
  g3->GetXaxis()->SetLimits(-2500,2500);
  g3->SetMinimum(-20);
  g3->SetMaximum(20);
  g3->Draw("p");
  canvas->Update();
  g1->Draw("psame");
  g2->Draw("psame");
  TF1 *f1 = new TF1("f1","[0]+[1]*x",-2500,2500);
  f1->SetLineStyle(2);
  ...
  g4->Draw("ap");
  g4->GetXaxis()->SetLimits(-2500,2500);
  g4->SetMinimum(-0.6);
  g4->SetMaximum(0.6);
  g5->Draw("psame");
  ...
  g6->Draw("ap");
  g6->GetXaxis()->SetLimits(-2500,2500);
  g6->SetMinimum(0);
  g6->SetMaximum(0.08);
  g7->Draw("psame");
  ...
  gBI->Draw("ap");
  TF1 *fBI = new TF1("fBI","pol6",0,2500);
  gBI->Fit(fBI,"R");
  ...
  betaRatioG->Draw("ap");
  TF1 *fBIrat = new TF1("fBIrat","pol6",100,2500);
  ...
  */
  ...
  TF1 *fleff = new TF1("fleff",fasdf,0,2500,5);
