double correctNegPol(double mkG){
  ...
  double corr = (-0.5352*pow(mkG,2) - 11.034*mkG - 0.8771)/1000;
//(1.2576E-06*pow(mkG,6) + 1.2046E-04*pow(mkG,5) + 4.4133E-03*pow(mkG,4) + 7.6389E-02*pow(mkG,3) + 8.0449E-02*pow(mkG,2) - 9.1638E+00*mkG)/1000;
  return mkG - corr;
...
double correctPosPol(double mkG){
  ...
  double corr = (-0.5389*pow(mkG,2) + 8.5914*mkG + 0.5901)/1000;
    //(1.1912E-06*pow(mkG,6) - 1.2134E-04*pow(mkG,5) + 4.7068E-03*pow(mkG,4) - 8.3844E-02*pow(mkG,3) + 9.9866E-02*pow(mkG,2) + 7.2453E+00*mkG)/1000;
  return mkG + corr;
...
void plotData(){
  ...
  */
  ...
  //make output pdf
  TFile *fout = new TFile("output/q3.root","RECREATE");
  TCanvas *canvas = new TCanvas("canvas","q3 data", 800, 800);
  ...
  std::string pdf_file_name = "output/q3.pdf";
  gROOT->SetBatch(true);
  ...
  cout<<"Bpos:\t"<<BmaxPos<<"\tBneg:\t"<<BmaxNeg<<endl;
  ...
  TText *t_asyn = new TText(100,0.04,"not probe error corrected");
  TText *t_asyc = new TText(100,0.05,"probe error corrected");
  ...
  gasy->Draw("ap");
  gasy->SetMinimum(-0.02);
  gasy->SetMaximum(0.06);
  gasyc->Draw("psame");
  t_asyn->Draw();
  t_asyc->Draw();
  canvas->Print( (pdf_file_name + "(").c_str());
  ...
  TText *t_asynr = new TText(500,0.0027,"not probe error corrected");
  ...
  //plot B/I vs I
  const double beta_g = 0.009728667;
  ...
  gBI->Draw("ap");
  TF1 *fBI = new TF1("fBI","pol6",0,4000);
  gBI->Fit(fBI,"R");
