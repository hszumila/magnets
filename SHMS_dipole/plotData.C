...
void plotData(){
  ...
  */
  ...
  //make output pdf
  TFile *fout = new TFile("output/dip.root","RECREATE");
  TCanvas *canvas = new TCanvas("canvas","dipole data", 800, 800);
  ...
  std::string pdf_file_name = "output/dip.pdf";
  gROOT->SetBatch(true);
  ...
  //plot B/I vs I
  const double beta_g = 0.011847889;
  ...
  betaRatioG->Draw("ap");
  TF1 *fBIrat = new TF1("fBIrat","pol6",0,4000);
